...
class Player(Entity):
    def __init__(self, x, y, ship_type=0):
        # ship_type 0: Interceptor, 1: Tank, 2: Scientific
        radius = [40, 50, 42][ship_type]
        color = [(30, 120, 255), (220, 40, 40), (255, 200, 50)][ship_type]
        super().__init__(x, y, radius=radius, color=color, mass=[1.2, 2.0, 1.5][ship_type], hp=[120, 250, 150][ship_type])
        self.ship_type = ship_type
        self.speed = [3500, 2200, 2800][ship_type]
        self.shoot_cooldown = 0.0
        self.max_shoot_cooldown = [0.1, 0.25, 0.15][ship_type]
        self.shield = 100
        self.max_shield = [80, 150, 120][ship_type]
        self.weapon_level = 1
        
        # Procedural points for fallback/mask
        self.points_list = [
            [(50, 0), (10, -15), (5, -35), (-25, -40), (-15, -15), (-35, -20), (-20, 0), (-35, 20), (-15, 15), (-25, 40), (5, 35), (10, 15)],
            [(45, 0), (35, -30), (10, -45), (-20, -50), (-40, -30), (-30, 0), (-40, 30), (-20, 50), (10, 45), (35, 30)],
            [(40, 0), (20, -20), (0, -40), (-20, -30), (-40, 0), (-20, 30), (0, 40), (20, 20)]
        ]
        self.points = self.points_list[ship_type]
    ...
    def handle_input(self, keys, dt, mouse_pos, camera_offset):
        ...
        if (keys[pygame.K_SPACE] or pygame.mouse.get_pressed()[0]) and self.shoot_cooldown <= 0:
            ...
            bullets = []
            bullet_color = (0, 255, 255) if self.ship_type != 1 else (255, 100, 50)
            
            if self.ship_type == 1: # Tank shoots heavy shots
                bullets.append(Bullet(self.pos.x, self.pos.y, self.angle, 1000, bullet_color, "player"))
                if self.weapon_level >= 2:
                    bullets.append(Bullet(self.pos.x, self.pos.y, self.angle - 0.1, 1000, bullet_color, "player"))
                    bullets.append(Bullet(self.pos.x, self.pos.y, self.angle + 0.1, 1000, bullet_color, "player"))
            else:
                if self.weapon_level >= 1:
                    bullets.append(Bullet(self.pos.x, self.pos.y, self.angle, 1300, bullet_color, "player"))
                if self.weapon_level >= 2:
                    off_x = 25 * math.cos(self.angle + math.pi/2)
                    off_y = 25 * math.sin(self.angle + math.pi/2)
                    bullets.append(Bullet(self.pos.x + off_x, self.pos.y + off_y, self.angle, 1300, bullet_color, "player"))
                    bullets.append(Bullet(self.pos.x - off_x, self.pos.y - off_y, self.angle, 1300, bullet_color, "player"))
            return bullets
        return None
    ...
    def draw(self, screen, camera_offset):
        ...
        engine_c = [1, 2, 3][self.ship_type]
        cockpit = [(255, 200, 0), (100, 255, 255), (255, 255, 255)][self.ship_type]
        self.draw_ship(screen, self.points, camera_offset, engine_count=engine_c, cockpit_color=cockpit)
